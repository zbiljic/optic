...
// vars for making sure that the compiler doesn't optimize out the benchmarks:
var (
	s      string
	I      interface{}
...
func BenchmarkNewParsedMetric(b *testing.B) {
	var mt optic.Metric
	for n := 0; n < b.N; n++ {
		mt, _ = NewParsed("test_metric",
			map[string]string{
				"test_tag_1": "tag_value_1",
				"test_tag_2": "tag_value_2",
				"test_tag_3": "tag_value_3",
...
func BenchmarkAddTag(b *testing.B) {
	...
	mt = &metric{
		...
		fields: map[string]interface{}{"a": float64(101)},
		ts:     time.Unix(0, 1480614053000000000),
	}
	for n := 0; n < b.N; n++ {
		mt.AddTag("foo", "bar")
	}
